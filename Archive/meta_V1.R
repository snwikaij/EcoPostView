...
meta <- function(estimate, stderr, parameter=NULL, predictor=NULL,
                ...
                moderator=NULL, method=0, RE=TRUE, Nsamp=NULL,
                prior_mu=0,
                prior_mu_se=10,
  ...
  if(!is.null(moderator) && is.data.frame(moderator)){
    if(nrow(moderator)!=length(estimate)){stop("Random  effect  needs to be a numeric data frame or matrix with
                                           length as the estimates")}}

  if(is.null(moderator)){ML <- 0; moderator <- NULL}else if(is.null(dim(moderator))){
    ML <- 1; moderator <- as.numer(moderator)}else{
    ML <- ncol(moderator); moderator[] <- lapply(moderator, as.factor);moderator  <- do.call(cbind, moderator)}
                                      ...
                                      if(RE==TRUE){
                                        if(mod_data$npw>1){
                                          ...
                                          meta_analysis <- function(){
                                              ...
                                              ##priors
                                              for(j in 1:L){
                                                beta_random[j] ~ dt(0, 1 / (0.9^2), 2.2)
                                                beta_adjust[j] ~ dt(0, 1 / (0.9^2), 2.2)
                                          ...
                                          meta_analysis <- function(){
                                              ...
                                              for(j in 1:L){
                                                for(r in 1:R){
                                                beta_random[j, r]  ~ dt(0, 1 / (0.9^2), 2.2)}
                                                beta_adjust[j]     ~ dt(0, 1 / (0.9^2), 2.2)
