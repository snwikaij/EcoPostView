...
$$\mu_{posterior}=\frac{\frac{\mu_{prior}}{n}+\hat{x}_{data}*\frac{1}{\sigma_{prior}^2}}
{\frac{1}{\sigma_{prior}^2}+\frac{1}{n}}$$
...
$$
If we now go back to how we analytically derived the posterior we can devise a function that can analytically perform a fixed effect meta-analysis with ease. I have placed this in a function called 'abmeta'. In in simple cases it approximates the results of metafor and and the meta function inf EcoPostView relatively well. Of course the variance component slightly differs with that from metafor due to the different method of estimation.
