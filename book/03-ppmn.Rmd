...
$$
This equation can be fitted with the `nls` package in R. See the example below

```{r, sigmoidal model fit}
...
$$
This equation can also be fitted with the `nls` package in R. 

```{r, gompertz model fit}
...
$$
x_c = Log(\frac{N(x_p | b0, b1)}{N(x_p | b2, b3)})\cdot Log(\frac{b4}{1-b4})
$$
