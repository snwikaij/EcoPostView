...
To use this package data is needed and on this data (G)LMs we need to fitted. This (meta-)data can be obtained from figures (e.g., using WebPlotDigitizer), tables (i.e., extrected by converting pdf to Excel files), data sets and combinations between them (See Kaijser et al. ...). The effects we are talking of are not really 'effects' but model parameters. These parameters are more commonly known as the intercept or slope. The intercept (*b0* or *β0*) and slope (*b1* or *β1*) can be used to predict using the following equation: *response variable = b0 + b1 · predictor variable*. By knowing *b0* and *b1* we can give the model a single or multiple new values (*i*) for *x* and make a prediction. However, in a GLM the response variable is linked to to the linear component with a link-function often notated as *g(..)*. The 'identity' link function applies no link function. This indicates that the mean (expected value of *y* notated as *E(y)* or *(E(y|x))*) is directly related to the linear component.
...
Heuristically, I prefer working often working with elasticity- or semi-elasticity coefficients (Woolridge, 2001). In this example I use the elasticity- or semi-elasticity coefficients. But, there are enough reasons not to do so and is dependent on the question, data and many other factors. 
...
An additional feature is the possibility to include the mean ($\hat{x}$) of the predictor gradient. This is later utilized in the display of the the results as Hypothetical Outcome Plots (HOPs) or in Partial Dependency plots (PDPs). This is optional but, advised because HOPs and PDPs are generate by given marginal predictions using one predictor variable holding all other predictor variables constant. If these values are not given the other variables are hold constant at the sum of the parameter estimations ($\sum_{j=1}^{j} \beta_j$). Note that the $log(\hat{x})$) is used, because the model were fit on log-transformed gradients to calculated the elasticity- or semi-elasticity coefficients. This is not exact, because the $\hat{x}(log(x))$ geometric mean is not the same as the same at the arithmetic mean $log(\hat{x})$. There are multiple options to handle this the simplest being directly deriving the $\hat{x}(log(x))$ from the data, but will address this later. This issues are however not so drastic in displaying the marginal changes.
...
mod1 <- meta(estimate=example1$est,         #Model estimate
             ...
             mu_predv=log(example1$mean))   #The log(mean) of the predictor variables (optional but advised)
