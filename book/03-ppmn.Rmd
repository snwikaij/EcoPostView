...
$$
This happens automatically when a GLM with log link and log-transformed independent variable (TP) was fitted. We just need a data frame (`example4`) with these columns (the columns `Source`, `Authors`, `Comment` could be omitted). In this data frame the point `estimate` is added and the `error`, plus the `edge` and `parameter`. The parameter b0 = $\beta_0$ and b1 = $\beta_1$. These are the minimal columns that need to be filled in in this data frame. The function that builds the PPMN will search for these names to construct the network.

Since the fitted GLMs had a log link the function (`fun`) is *log*, the edge-functions (`edge`) name is *Chl~TP* and we also transformed (`trans`) the independent variable and therefore use *log*. If a log transformation was not possible and a square-root transformation was used *sqrt* can be used. If no transformation has been applied the argument does not have to be used and can be omitted. Other transformations can be fitted on request if needed. 
...
or as a directed Bipartite Network where the boxes represent the functions and the variables the circles.
...
A slightly more complex example would to regress *Chl~TN+TP* and use Chl to predict Taxa richness of phytoplankton algae *Taxa~Chl*
...
$$
The full network would then be expressed as

$$
f_{Taxa|Chl}(f_{Chl|TN, TP}(x_{TN}, x_{TP}, \theta_{Chl|TN,TP,1},  \theta_{Chl|TN,TP,2}), \theta_{Taxa|Chl})
$$
The formula should then be specified as below. A minor addition is the inclusion of two priors for the edge *Taxa~Chl*. Assume there are two individuals that generated a prior and then both sources of information may be added. Directly after building the foundational PPMN we choose to display the graphical representation.
...
We might also wish display the marginal relations of a single edge-function of the model. Some data is created able to display the marginal distribution of the edge-function. This marginal relation is an added variable plot.
