...
#' @param warnings An argument that if True returns a warning is the number of studies is 1 (default warnings = FALSE)
#'
#' @description
...
abmeta <- function(estimate, stderr, prior_mu=0, prior_mu_se=1000, prior_weights=NULL,
  ...
  if(length(prior_mu) > 1 && !is.null(prior_weights) && length(prior_weights) != length(prior_mu)){
    prior_weights <- rep(1/length(prior_mu), length(prior_mu))}
  ...
  if (RE == T){
    ...
    posteriors <- sapply(1:length(prior_mu), function(k) {
      postvals(estimate, sqrt(stderr^2 + tau2), prior_mu[k], prior_mu_se[k], a, b)})
