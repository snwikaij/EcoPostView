...
```{r, apply updated pppn to validation data}
...
```{r, root estimation monte carlo priors, fig.width=8, fig.height=4, echo=F, warning=FALSE}
...
```{r, figure 15 macrophytes, fig.width=8, fig.height=5, echo=F, warning=FALSE}
...
```{r, figure 1 macrophyte, fig.width=8, fig.height=5, echo=F, warning=FALSE}
...
There are different functions that can be used in the PPMN. These are not limited to the ones provided here. Upon request I can add different functions that might be useful for the user. Of course the standard functions consist of the linear equation and link functions used with GLMs. The R-package express the equation using "b" ($\beta$ or $\theta$) for "b0" ($\beta_0$ or $\theta_0$) or "b1" ($\beta_1$ or $\theta_1$) etc. Each parameter in the equation follow the notation b0, b1, b2, ..., etc.

### Linear equation

For the linear equation equation the standard expression is used of which the name is `identity`. Where $k\in K$ describes the number of parameters.

$$
x_c=b0+\sum_{k=1}^K(b_k\cdot x_k)
$$

### Log-linear equation

For the log-linear equation equation the standard expression is used of which the name is `log`.

$$
log(x_c)=b0+\sum_{k=1}^K(b_k\cdot x_k)
$$

### Logit-linear equation

For the log-linear equation equation the standard expression is used of which the name is `logit`.

$$
log(\frac{x_c}{(1-x_c)})=b0+\sum_{k=1}^K(b_k\cdot x_k)
$$### Logit-linear equation

### Sigmoidal equation

For the three parameter sigmoidal equation used of which the name is `sigmoidal`. b0 describes the asymptote, b1 the midpoint and b2 the scale parameter.

$$
x_c=\frac{b0}{(1+exp(\frac{(b1-x_k)}{b2}))}
$$

### Gompertz equation

For the gompertz equation the name is `gompertz`. In this equation, b0 is the asymptote, b1 is the displacement constant and b2 is the growth rate.

$$
x_c=b0 \cdot exp(-exp( b1 - b2 \cdot x_k)))
$$
...
$$
x_c = b0 * exp(-0.5 * (\frac{(x - b1)}{b2})^2)
$$

### Bayesian classifier

The Bayesian classifier is an equation that makes use of the mean and standard deviation of two groups. In this equation b0 and b1 are the mean and standard deviation of group 1, respectively. b2 and b3 are the mean and standard deviation of group 2. b5 is the proportion of group 1.

$$
x_c = Log(\frac{N(x_p | b0, b1)}{N(x_p | b0, b1)})\cdot Log(\frac{b5}{1-b5})
$$
