...
A simple syllogistic argument consisting of two premises. This argument is names "modens ponens" having a specific structure.
...
$$
In an argument all premises in the argument being declarative statements defined as propositions, which are either True or False. Each premise consists of the antecedent (subject; P), copula (is, am, will, etc.) and consequent (Q). 

A valid argument is an argument where if all propositions are to be true the conclusion cannot be false. In propositional logic the argument as given before can be formalized  expressed as below where indicates and implication as ‘implies’ and the three dots indicating therefore. 
...
$$for \ all \ x \ that \ are P \ it \ implies \ they \ are \ Q, x \ is \ P \ therefore \ Q\\
for \ all \ x \ that \ are \ P \ it \ implies \ they \ are \ Q, x \ is \ \neg P \ therefore \ \neg Q\\
for \ all \ x \ that \ are \ P \ it \ implies \ they \ are \ Q, for \ all \ x \ that \ are \ Q \ it \ implies \ they \ are \ Z, \ x \ is \ P \ therefore \ Z \\
either \ x \ is \ P \ or \ x \ is \ Q, x \ is \ P \ therefore \ \neg Q$$
