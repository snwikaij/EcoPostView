...
The derivation of Bayes' theorem relies on the ex changeability of probabilities, while classically `A` and `B` are used for reference to the text where `A = \beta` and `B = Data`.
...
```{r fig log_sal1, fig.width=7, fig.height=4, echo=FALSE}
print(log_sal1)
```

It is also possible to scale to display the exponentiate values on the x-axis using the argument
"exp_axis". The observant reader might notice the taxonomic richness is ~15 under lower conductivity. This is correct, because the majority of the literature publish does not focus on species richnes, it often focuses on order or family richness.
...
```{r fig log_sal2, fig.width=7, fig.height=4, echo=FALSE}
print(log_sal2)
```
...
log_sal3 <- hop(mod2,                                              
    group="Invertebrates",                            
    predictor = c("Salinity", "Oxygen"),                     #select both Salinity and Oxygen
    xlab= expression(Conductivity ~ µS ~ cm^-1),             #give x-axis a name
...
```{r fig log_sal3, fig.width=7, fig.height=4, echo=FALSE}
print(log_sal3)
```
