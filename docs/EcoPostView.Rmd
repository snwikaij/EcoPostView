...
Below a simplified example of how the choice of the prior influences the output of the posterior. In this example the prior mean, standard error and skew can be adjusted. Similar, the mean and the standard error of the likelihood can be adjusted. Based on these adjustments the posterior is shaped.
...
ui <- fluidPage(
  ...
  mainPanel(
    ...
    fluidRow(
      column(6, 
        div(class = "slider-container", 
          sliderInput(inputId = "slider_prior_mu",
                      label = "Prior mean",
                      min = -3, max = 3, value = 0, step = 0.01)),
      ),
      column(6,
        div(class = "slider-container", 
          sliderInput(inputId = "slider_prior_se",
                      label = "Prior standard error",
    ...
    fluidRow(
      column(6, 
        div(class = "slider-container", 
          sliderInput(inputId = "slider_prior_skew",
                      label = "Prior skew",
                      min = -3, max = 3, value = 0, step = 0.01))
      ),
      column(6,
        div(class = "slider-container", 
          sliderInput(inputId = "slider_like_mu",
                      label = "Likelihood mean (data)",
    ...
    fluidRow(
      column(6, 
        div(class = "slider-container", 
          sliderInput(inputId = "slider_like_se",
                      label = "Likelihood standard error (data)",
                      min = 0.01, max = 2, value = 0.5, step = 0.01))
      )
    ),
    fluidRow(
      column(12,
        plotOutput("Bayes_example", height = "600px", width = "100%")
      )
    )
  )
)
...
server <- function(input, output) {
  ...
  output$Bayes_example <- renderPlot({
    
    xgrid      <- seq(-5, 5, 0.01)
    prior      <- VGAM::dskewnorm(xgrid, location = input$slider_prior_mu,
                                  scale = input$slider_prior_se,
                                  shape = input$slider_prior_skew)
    
    likelihood <- dnorm(xgrid, input$slider_like_mu, input$slider_like_se)
    posterior  <- likelihood * prior / max(likelihood * prior)

    prior_samp      <- sample(xgrid, 3000, T, prob = prior)
    like_samp       <- sample(xgrid, 3000, T, prob = likelihood)
    post_samp       <- sample(xgrid, 3000, T, prob = posterior)
    
    text_prior      <- paste0("Prior mean=f(β|Info)=", round(mean(prior_samp), 2),
                               " (5%=", round(quantile(prior_samp, .05), 2),
                               ", 95%=", round(quantile(prior_samp, .95), 2), ")")
    
    text_like       <- paste0("Likelihood mean=f(Data|β)=", round(mean(like_samp), 2),
                               " (5%=", round(quantile(like_samp, .05), 2),
                               ", 95%=", round(quantile(like_samp, .95), 2), ")")
    
    text_post       <- paste0("Posterior mean=f(β|Data, Info)=", round(mean(post_samp), 2),
                               " (5%=", round(quantile(post_samp, .05), 2),
                               ", 95%=", round(quantile(post_samp, .95), 2), ")")

    display <- data.frame(term = rep(c(text_prior, text_like, text_post), each = length(xgrid)),
                          x = c(xgrid, xgrid, xgrid),
                          y = c(prior, likelihood, posterior))

    display$term <- factor(display$term, levels = c(text_prior, text_like, text_post))

    ggplot(display, aes(x = x, y = y)) +
      geom_line() + xlab("β") +
      geom_vline(xintercept = 0, col = "tomato3", lty = 2) +
